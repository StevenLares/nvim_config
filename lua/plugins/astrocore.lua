...
return {
  ...
  opts = {
    ...
    -- Disables macro recording with q, and moves it to Leader key
    autocmds = {
      smart_q = {
        {
          event = "VimEnter",
          ...
          callback = function()
            ...
            vim.keymap.set("n", "<Leader>0", function()
              ...
              if recording == "" then
                ...
                vim.notify("Started recording macro @m", vim.log.levels.INFO, { title = "Macro" })
              else
                -- stop recording
                vim.api.nvim_feedkeys("q", "n", false)
                vim.notify("Stopped recording macro @m", vim.log.levels.INFO, { title = "Macro" })
              end
      ...
      -- autocmd group, that adds the ability to switch to normal mode in terminal:
      terminal_keymaps = {
        {
          event = "TermOpen",
          pattern = "term://*",
